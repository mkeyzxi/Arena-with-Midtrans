...
import '../services/firebase_service.dart'; // gunakan service kamu
...
class _AdminBookingScreenState extends State<AdminBookingScreen> {
  ...
  Future<void> _pickDate() async {
    final r = await showDatePicker(
      context: context,
      firstDate: DateTime.now(), // ✅ cukup sekali
  ...
  bool _checkSlotAvailability(Field field) {
    ...
    // Di dalam jam operasional
    final openTime = DateTime(
      _selectedDate.year,
      _selectedDate.month,
      _selectedDate.day,
      int.parse(field.openHour.split(':')[0]),
      int.parse(field.openHour.split(':')[1]),
    );
    final closeTime = DateTime(
      ...
      int.parse(field.closeHour.split(':')[1]),
    );
  ...
  Future<void> _submit(Field field) async {
    if (!(_isSlotAvailable ?? false) ||
        (_selectedUser == null && _manualUsernameCtl.text.trim().isEmpty)) {
    ...
    final String customerEmail =
        _selectedUser?.email ?? 'guest@arena.com'; // default guest
    ...
    final booking = Booking(
      ...
      status: 'pending', // nanti diupdate setelah createTransaction
              ...
              FutureBuilder<List<User>>(
                ...
                builder: (_, userSnap) {
                  ...
                  if (userSnap.hasError) {
                    return Text('Gagal memuat user: ${userSnap.error}');
                  }
